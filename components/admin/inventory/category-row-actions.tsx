...
export function InventoryCategoryRowActions({
  category,
}: InventoryCategoryRowActionsProps) {
          ...
          <form
            ...
            onSubmit={(event) => {
              const confirmed = window.confirm(
                `Видалити категорію складу «${category.name}»? Цю дію не можна скасувати.`,
              );
              if (!confirmed) {
    ...
    <article className="rounded-2xl border border-[#c9a227]/40 bg-[#fffaf3] p-4">
      ...
      <form action={updateAction} className="mt-4 grid gap-4">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <div>
            <label
              htmlFor={`inventory-name-${category.id}`}
              className="block text-sm font-medium text-[#3c2a21]"
            >
              Назва
            </label>
            <input
              id={`inventory-name-${category.id}`}
              name="name"
              type="text"
              required
              maxLength={80}
          ...
          <div>
            <label
              htmlFor={`inventory-slug-${category.id}`}
              className="block text-sm font-medium text-[#3c2a21]"
            >
              Slug
            </label>
            <input
              id={`inventory-slug-${category.id}`}
              name="slug"
              type="text"
              required
        ...
        <div className="grid gap-4 sm:grid-cols-[140px_1fr] sm:items-end">
          <div>
            <label
              htmlFor={`inventory-sort-${category.id}`}
              className="block text-sm font-medium text-[#3c2a21]"
            >
              Порядок
            </label>
            <input
              id={`inventory-sort-${category.id}`}
              name="sortOrder"
              type="number"
              min={0}
              max={10000}

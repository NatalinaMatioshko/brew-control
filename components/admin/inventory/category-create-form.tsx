...
const initialState: InventoryCategoryActionResult | null = null;

export function InventoryCategoryCreateForm() {
  ...
  return (
    <section className="rounded-2xl border border-[#e4d5c5] bg-[#fbf6f0] p-5">
      <h2 className="text-lg font-semibold text-[#3c2a21]">Додати категорію складу</h2>
      <p className="mt-1 text-sm text-[#5c4638]">
        Наприклад: зерно, молоко, сиропи, стакани. Slug згенерується з назви.
      </p>

      <form
        ref={formRef}
        action={formAction}
        className="mt-4 grid gap-4 sm:grid-cols-[1fr_140px_auto] sm:items-end"
      >
        <div>
          <label
            htmlFor="inventory-category-name"
        ...
        <div>
          <label
            htmlFor="inventory-category-sort-order"
            className="block text-sm font-medium text-[#3c2a21]"
          >
            Порядок
          </label>
          <input
            id="inventory-category-sort-order"
            name="sortOrder"
            type="number"
            min={0}
            max={10000}
